...
class Constra_Nav_Menu extends Walker
{
    /**
    ...
    public function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0)
    {
        ...
        $menu_has_children = !empty($menu_item->classes) && in_array('menu-item-has-children', $menu_item->classes);

        if ($menu_has_children && $depth === 0) {
            $classes[] = 'dropdown';
        }

        // Apply 'dropdown-submenu' only if it's a submenu with children
        if ($menu_has_children && $depth > 0) {
        ...
        if ($depth === 0) {
            ...
            if ($menu_has_children) {
                $atts['class'] = isset($atts['class']) ? 'nav-link dropdown-toggle' : 'nav-link';

                $atts['after'] = ' <i class="fa fa-angle-down"></i>';
            }
        } else {
            $atts['class'] = '';
        }
        ...
        // Append the <i> tag directly if the menu has children
        if ($menu_has_children && $depth === 0) {
